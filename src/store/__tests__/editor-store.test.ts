...
describe('editor-store', () => {
  ...
  describe('setActiveSlideIndex', () => {
    it('changes the active slide and clears selection', () => {
      ...
      useEditorStore.getState().setSelectedElement({
        selector: 'p', tagName: 'P', className: '', text: 'test',
  ...
  describe('updateSlideHtml', () => {
    it('updates the slide outerHtml and marks dirty', () => {
      useEditorStore.getState().setPresentation(makePresentation());
      useEditorStore.getState().updateSlideHtml('slide-1', '<section>Updated</section>');
      const state = useEditorStore.getState();
      expect(state.presentation!.slides[1].outerHtml).toBe('<section>Updated</section>');
      expect(state.isDirty).toBe(true);
    });

    it('pushes to undo stack', () => {
      useEditorStore.getState().setPresentation(makePresentation());
      useEditorStore.getState().updateSlideHtml('slide-0', '<section>Edit 1</section>');
    ...
    it('clears redo stack on new edit', () => {
      ...
      useEditorStore.getState().updateSlideHtml('slide-0', '<section>Edit 2</section>');
  ...
  describe('undo / redo', () => {
    it('undo restores previous slide state', () => {
      useEditorStore.getState().setPresentation(makePresentation());
      const originalHtml = useEditorStore.getState().presentation!.slides[0].outerHtml;
      useEditorStore.getState().updateSlideHtml('slide-0', '<section>Changed</section>');
      useEditorStore.getState().undo();
      expect(useEditorStore.getState().presentation!.slides[0].outerHtml).toBe(originalHtml);
    });
    ...
    it('redo restores the undone change', () => {
      ...
      expect(useEditorStore.getState().presentation!.slides[0].outerHtml).toBe('<section>Changed</section>');
    });

    it('multiple undos walk back through history', () => {
      useEditorStore.getState().setPresentation(makePresentation());
      const original = useEditorStore.getState().presentation!.slides[0].outerHtml;
      useEditorStore.getState().updateSlideHtml('slide-0', '<section>Edit 1</section>');
      useEditorStore.getState().updateSlideHtml('slide-0', '<section>Edit 2</section>');
      useEditorStore.getState().updateSlideHtml('slide-0', '<section>Edit 3</section>');
      ...
      expect(useEditorStore.getState().presentation!.slides[0].outerHtml).toBe(original);

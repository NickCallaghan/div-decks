...
import type { PresentationFile, PresentationModel, SlideModel } from '../types/presentation';
...
function pushHistory(state: EditorState): Pick<EditorState, 'undoStack' | 'redoStack' | 'canUndo' | 'canRedo'> {
  if (!state.presentation) return { undoStack: state.undoStack, redoStack: state.redoStack, canUndo: state.canUndo, canRedo: state.canRedo };
